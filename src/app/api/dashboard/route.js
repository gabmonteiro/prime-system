  ...
  try {
    ...
    switch (period) {
      case "week":
        ...
        break;
      case "month":
        // Para o dashboard, vamos retornar dados dos últimos 12 meses para permitir seleção de meses anteriores
      ...
      case "year":
        ...
        break;
      default:
        // Padrão: últimos 12 meses para permitir seleção de meses anteriores
        startDate = new Date(now.getFullYear(), now.getMonth() - 11, 1);
    }
    ...
    if (checkPermission(user, "servicos", "read")) {
      ...
      const servicosValue = servicos.reduce((total, servico) => {
        const valor =
          servico.valorPersonalizado || servico.tipoServico?.valor || 0;
    ...
    if (
      checkPermission(user, "servicos", "read") &&
      checkPermission(user, "despesas", "read")
    ) {
      ...
      const servicosTotal = await Servico.find({ pago: true })
        .populate("tipoServico")
        .lean();
      ...
      const servicosValorTotal = servicosTotal.reduce((total, servico) => {
        const valor =
          ...
          (servico.tipoServico ? servico.tipoServico.valor : 0);
      ...
      if (servicosTotal.length > 0) {
        console.log(
          ...
          servicosTotal.slice(0, 3).map((s) => ({
            id: s._id,
            cliente: s.cliente,
            valorPersonalizado: s.valorPersonalizado,
            tipoServico: s.tipoServico
              ? { id: s.tipoServico._id, valor: s.tipoServico.valor }
              : null,
            valorCalculado:
              s.valorPersonalizado || (s.tipoServico ? s.tipoServico.valor : 0),
          })),
        );
      }
    }
